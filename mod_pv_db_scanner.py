...
class ModPvDbScanner:
    ...
    def get_all_songs(self):
        ...
        for mod_pv_db_path in self.root_folder.rglob('**/mod_pv_db.txt'):
            ...
            song_pack = self.get_song_pack_name(Path(f'{mod_pv_db_path.parents[1]}/config.toml'))
    ...
    @staticmethod
    def read_song_names(mod_pv_db_path):
        song_names = set()
    ...
    @staticmethod
    def has_script_folder(directory):
        script_directory_path = directory / 'script'
        return script_directory_path.exists()

    @staticmethod
    def get_song_pack_name(config_path):
        try:
            with config_path.open('r', encoding='utf-8') as f:
                config_data = toml.load(f)
                return config_data.get('name') if config_data.get('name') else config_path.parent.name
